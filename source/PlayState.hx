...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		else
		{
			defaultCamZoom = 0.9;
			curStage = 'stage';
			bgs = new FlxSprite(-600, -200).loadGraphic(Paths.image("stageback"));
			// bgs.setGraphicSize(Std.int(bg.width * 2.5));
			// bgs.updateHitbox();
	...
	override function stepHit()
	{
		...
		if (curSong == 'Science-Blaster')
			{
				switch (curStep)
				{
					case 512:
						remove(bgs);
						bgs = new FlxSprite(-600, -200).loadGraphic(Paths.image("stageback2"));
						add(bgs);
						trace('SUS');
					case 768:
						remove(bgs);
